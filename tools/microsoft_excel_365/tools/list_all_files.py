        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if filtered_items:
                    ...
                    # Create summary text
                    summary = f"Found {len(filtered_items)} item(s) in '{folder_path}':\n\n"
                    ...
                    if excel_files:
                        ...
                        if len(excel_files) > 10:
                            summary += f"  ... and {len(excel_files) - 10} more Excel files\n"
                        summary += "\n"
                    ...
                    yield self.create_text_message(f"No items found in '{folder_path}'.")
                    
            elif response.status_code == 401:
                yield self.create_text_message("Authentication failed. Access token may be expired.")
            elif response.status_code == 404:
                yield self.create_text_message(f"The path '{folder_path}' was not found.")
            else:
                yield self.create_text_message(f"Failed to list files: {response.status_code} - {response.text}")
