...
class ClearWorksheetDataTool(Tool):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        try:
            ...
            payload = {
                "applyTo": "Contents"  # Clear only contents, not formatting
            }
            ...
            if response.status_code == 200:
                ...
                yield self.create_text_message(f"Successfully cleared data from range '{range_address}' in worksheet '{worksheet_name}'.")
                ...
                yield self.create_text_message(f"Bad request. Please check the range format.")
            else:
                yield self.create_text_message(f"Failed to clear worksheet data: {response.status_code} - {response.text}")
