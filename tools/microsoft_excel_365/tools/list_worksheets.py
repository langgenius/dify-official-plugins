            ...
            if response.status_code == 200:
                ...
                if worksheets:
                    ...
                    for ws in worksheets:
                        visibility = f" [{ws['visibility']}]" if ws['visibility'] != 'Visible' else ""
                        summary += f"{ws['position'] + 1}. {ws['name']}{visibility} (ID: {ws['id']})\n"
                    yield self.create_text_message(summary)
                else:
                    yield self.create_text_message("No worksheets found in the workbook.")
                ...
                yield self.create_text_message(f"Failed to list worksheets: {response.status_code} - {response.text}")
