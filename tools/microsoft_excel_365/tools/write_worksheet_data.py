    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        if isinstance(values, str):
            try:
                ...
                yield self.create_text_message("Invalid values format. Please provide a valid JSON array.")
                return
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                yield self.create_text_message(f"Bad request. Please check the range and values format.")
            else:
                yield self.create_text_message(f"Failed to write worksheet data: {response.status_code} - {response.text}")
