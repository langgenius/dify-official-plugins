    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                yield self.create_text_message(f"Successfully created new worksheet '{worksheet_name}' (ID: {worksheet_info['id']}) at position {worksheet_info['position'] + 1}.")
                    
            elif response.status_code == 401:
                yield self.create_text_message("Authentication failed. Access token may be expired.")
            elif response.status_code == 404:
                yield self.create_text_message(f"Workbook with ID '{workbook_id}' not found.")
            elif response.status_code == 400:
                error_msg = response.json().get("error", {}).get("message", "Bad request")
                if "already exists" in error_msg.lower():
                    yield self.create_text_message(f"A worksheet with the name '{worksheet_name}' already exists in this workbook.")
                else:
                    yield self.create_text_message(f"Bad request: {error_msg}")
            else:
                yield self.create_text_message(f"Failed to create worksheet: {response.status_code} - {response.text}")
