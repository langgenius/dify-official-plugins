...
class ListWorkbooksTool(Tool):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        if not access_token:
            yield self.create_text_message("Access token is missing. Please authenticate first.")
            return
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for item in items:
                    if item.get("file") and item.get("name", "").lower().endswith(('.xlsx', '.xls', '.xlsm', '.xlsb')):
                        workbook_info = {
                            "id": item["id"],
                            "name": item["name"],
                ...
                if workbooks:
                    ...
                    if search_query:
                        yield self.create_text_message(f"No Excel workbooks found matching '{search_query}'.")
                    elif folder_id:
                        yield self.create_text_message(f"No Excel workbooks found in the specified folder.")
                    else:
                        yield self.create_text_message("No Excel workbooks found. Try uploading an Excel file to OneDrive first.")
                    
            elif response.status_code == 401:
                yield self.create_text_message("Authentication failed. Access token may be expired.")
            elif response.status_code == 404:
                yield self.create_text_message("The specified folder was not found.")
            else:
                yield self.create_text_message(f"Failed to list workbooks: {response.status_code} - {response.text}")
