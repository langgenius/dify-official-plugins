...
- In self-hosted deployments, tool nodes run inside the plugin runtime (plugin-daemon). Ensure it can reach the configured Base URL (network egress / proxy / DNS). Avoid using `localhost` unless the service is running in the same container.

- API Key 和 Base URL 由插件配置自动注入，**无需**在节点中手动输入。
- 私有化部署时，工具节点在插件运行时（plugin-daemon）中执行。请确保该运行环境能够访问你配置的 Base URL（出网 / 代理 / DNS）。除非服务就在同一容器内，否则不要使用 `localhost`。
