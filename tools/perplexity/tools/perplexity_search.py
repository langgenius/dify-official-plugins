import json
from typing import Any, Generator

import requests
from dify_plugin import Tool
from dify_plugin.entities.tool import ToolInvokeMessage

PERPLEXITY_API_URL = "https://api.perplexity.ai/v1/agent"


class PerplexityAITool(Tool):
    def _parse_response(self, response: dict) -> dict:
        """Parse the response from the Perplexity Agent API"""
        citations = []
        for item in response.get("output", []):
            if item.get("type") == "search_results":
                citations.extend(result.get("url", "") for result in item.get("results", []))

        return {
            "content": response.get("output_text", ""),
            "role": "assistant",
            "citations": citations,
        }

    def _invoke(
        self, tool_parameters: dict[str, Any]
    ) -> Generator[ToolInvokeMessage, None, None]:
        headers = {
            "Authorization": f"Bearer {self.runtime.credentials['perplexity_api_key']}",
            "Content-Type": "application/json",
            "X-Pplx-Integration": "dify",
        }

        web_search_tool: dict[str, Any] = {
            "type": "web_search",
            "search_context_size": tool_parameters.get("search_context_size", "low"),
        }

        filters: dict[str, Any] = {}
        if tool_parameters.get("search_domain_filter"):
            filters["search_domain_filter"] = [
                domain.strip() for domain in tool_parameters["search_domain_filter"].split(",")
            ][:20]
        if tool_parameters.get("search_recency_filter"):
            filters["search_recency_filter"] = tool_parameters["search_recency_filter"]
        if filters:
            web_search_tool["filters"] = filters

        payload = {
            "preset": tool_parameters.get("preset", "fast"),
            "input": tool_parameters["query"],
            "instructions": "Be precise and concise.",
            "tools": [web_search_tool],
            "temperature": tool_parameters.get("temperature", 0.7),
            "top_p": tool_parameters.get("top_p", 1),
            "top_k": tool_parameters.get("top_k", 0),
            "presence_penalty": tool_parameters.get("presence_penalty", 0),
            "frequency_penalty": tool_parameters.get("frequency_penalty", 1),
            "max_output_tokens": tool_parameters.get("max_tokens", 4096),
            "streaming": False,
        }

        response = requests.post(url=PERPLEXITY_API_URL, json=payload, headers=headers)
        response.raise_for_status()
        valuable_res = self._parse_response(response.json())
        for resp in [
            self.create_json_message(valuable_res),
            self.create_text_message(json.dumps(valuable_res, ensure_ascii=False, indent=2)),
        ]:
            yield resp
