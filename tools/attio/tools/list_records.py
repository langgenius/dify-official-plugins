...
class listRecordsTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        try:
            # Get credentials
            url = f"https://api.attio.com/v2/objects/{object_slug}/records/query"
            ...
            if response.status_code == 400:
                ...
                yield self.create_text_message(f"Unauthorized request - {response.text}")
                return
            elif response.status_code == 404:
                yield self.create_text_message(f"Objects not found - {response.text}")
                return
            elif response.status_code != 200:
                yield self.create_text_message(f"Failed to retrieve records: {response.text}")
                return

            yield self.create_text_message("Records retrieved successfully.")
            ...
            yield self.create_text_message("Failed to connect to Attio - please check your configuration")
