...
class PresentationGeneratorTool(Tool):
    ...
    def _invoke(
        ...
        if isinstance(document_uuids_param, str):
            try:
                ...
                if isinstance(loaded, list):
                    ...
                    yield self.create_text_message("Error: document_uuids must be a JSON array of strings")
                    return
            except json.JSONDecodeError:
                yield self.create_text_message("Error: document_uuids must be a valid JSON string")
                return
        elif document_uuids_param and not isinstance(document_uuids_param, list):
            yield self.create_text_message("Error: document_uuids must be a list or JSON string")
            return
        ...
        if not request.plain_text and not request.document_uuids:
            yield self.create_text_message("Error: Either plain_text or document_uuids is required")
            return
        ...
        try:
            ...
            yield self.create_blob_message(
                ...
                meta={"mime_type": "application/pdf" if request.response_format == "pdf" else "application/vnd.openxmlformats-officedocument.presentationml.presentation"},
            )
