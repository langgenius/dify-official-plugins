...
class TemplateListerTool(Tool):
    ...
    def _invoke(
        ...
        try:
            ...
            markdown_content = f"# SlideSpeak Templates\n\nFound {len(templates)} templates.\n\n"
            
            for template in templates:
                markdown_content += f"## {template.name}\n\n"
                markdown_content += f"**Cover Image:**\n"
                markdown_content += f"![{template.name} Cover]({template.images.cover})\n\n"
                markdown_content += f"**Content Image:**\n"
                markdown_content += f"![{template.name} Content]({template.images.content})\n\n"
                markdown_content += "---\n\n"
