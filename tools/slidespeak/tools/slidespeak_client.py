...
class SlideSpeakClient:
    ...
    def __init__(self, api_key: str, base_url: str = "https://api.slidespeak.co/api/v1", 
                 timeout: float = 100.0, poll_interval: int = 2):
        """
        ...
        """
        if not api_key:
            raise ToolProviderCredentialValidationError("SlideSpeak API key is required")
    ...
    def _log_request(self, method: str, url: str, headers: Dict[str, str], body: Any = None, files: Any = None):
        """Log request details"""
        print(f"\n=== REQUEST ===")
    ...
    def generate_presentation_slide_by_slide(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def generate_and_fetch_presentation(self, request_data: Dict[str, Any]) -> tuple[str, bytes]:
        """
    ...
    def generate_and_fetch_presentation_slide_by_slide(self, request_data: Dict[str, Any]) -> tuple[str, bytes]:
        """
    ...
    def upload_document_and_get_uuid(self, file_bytes: bytes, filename: str) -> str:
        ...
        while True:
            ...
            if task_status == TaskState.FAILURE:
                raise Exception(f"Document upload failed with status: {task_status.value}")
