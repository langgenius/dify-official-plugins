...
class MineruTool(Tool):
    ...
    def _validate_local_server(self, credentials: Credentials) -> None:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Local server validation failed with status {response.status_code}"
                )
                raise ToolProviderCredentialValidationError(
                    "Please check your base_url"
                )
    ...
    def _validate_remote_server(self, credentials: Credentials) -> None:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Remote server validation failed with status {response.status_code}"
                )
                raise ToolProviderCredentialValidationError(
                    "Please check your base_url and token"
                )
        except Exception as e:
    ...
    def _process_base64_image(
        self, encoded_image_data: str, file_name: str
    ) -> UploadFileResponse:
        try:
    ...
    def _parse_local_v1(
        ...
        if response.status_code != 200:
            ...
            yield self.create_text_message(
                f"Failed to parse file. result: {response.text}"
            )
            return
    ...
    def _parse_local_v2(
        ...
        lang_list = ["ch"]
        if (
            tool_parameters.get("language")
            and tool_parameters.get("language") != "auto"
        ):
        ...
        for file_name, result in results.items():
            ...
            if result.get("images"):
                ...
                for img_name, encoded_image_data in result["images"].items():
                    try:
                        file_res = self._process_base64_image(
                            encoded_image_data, img_name
                        )
            ...
            if result.get("content_list"):
                try:
                    ...
                    yield self.create_json_message(
                        {"content_list": result_item["content_list"]}
                    )
    ...
    def _parser_file_remote(
        ...
        if response.status_code != 200:
            logger.error(
                f"Apply upload url failed. status:{response.status_code}, result:{response.text}"
            )
            raise Exception(
                f"Apply upload url failed. status:{response.status_code}, result:{response.text}"
            )
        ...
        if result["code"] == 0:
            ...
            logger.error(
                f"Apply upload url failed, reason:{result.get('msg', 'Unknown error')}"
            )
            raise Exception(
                f"Apply upload url failed, reason:{result.get('msg', 'Unknown error')}"
            )

    def _poll_get_parse_result(
        self, credentials: Credentials, batch_id: str
    ) -> Dict[str, Any]:
        """poll get parser result."""
        url = self._build_api_url(
            credentials.base_url, f"api/v4/extract-results/batch/{batch_id}"
        )
        headers = self._get_headers(credentials)
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Failed to get parse result, status: {response.status_code}"
                    )
    ...
    def _download_and_extract_zip(
        ...
        content.md_content = self._replace_md_img_path(
            content.md_content, content.images
        )
    ...
    def _process_zip_file(
        ...
        try:
            if file_name.startswith("images/") and file_name.endswith(
    ...
    def _process_image(
        self, image_bytes: bytes, file_info: zipfile.ZipInfo
    ) -> UploadFileResponse:
    ...
    def _replace_md_img_path(md_content: str, images: List[UploadFileResponse]) -> str:
        """Replace image path in Markdown and fix image format."""
        import re

        # Fix empty alt text to "img" in markdown images
        # Replace ![](images/ with ![img](images/
        md_content = re.sub(r"!\[\]\(images/", r"![image](images/", md_content)

        # Then replace image paths with URLs
        for image in images:
            if image.preview_url:
                md_content = md_content.replace(
                    f"images/{image.name}", image.preview_url
                )
        return md_content
