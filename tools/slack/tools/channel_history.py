...
class ChannelHistoryTool(Tool):
    def _invoke(
        ...
        params: dict[str, Any] = {
            "channel": channel,
            "oldest": tool_parameters.get("oldest"),
            "latest": tool_parameters.get("latest"),
            "limit": tool_parameters.get("limit") or 100,
            "cursor": tool_parameters.get("cursor"),
        }

        try:
            response = client.call("conversations.history", "GET", params=params)
