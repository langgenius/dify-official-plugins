...
class RenameChannelTool(Tool):
    def _invoke(
        ...
        if not channel or not name:
            yield self.create_text_message("Channel and New Name are required.")
            return
        ...
        if not ok:
            ...
            return
        yield self.create_text_message(f"Channel {channel} renamed to '{name}'.")
