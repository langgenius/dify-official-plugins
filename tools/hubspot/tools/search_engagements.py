...
class SearchEngagementsTool(Tool):
    """Search / list engagements with optional query, filter and sort."""
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        OBJECT = tool_parameters.get("engagement_type") or "notes"
        ...
        yield self.create_text_message(
            f"Found {len(results)} {OBJECT}" + (f" (total {total})." if total is not None else ".")
        )
