...
class CreateTicketTool(Tool):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        if not properties:
            yield self.create_text_message("Provide at least one property (e.g. subject).")
            return

        try:
            result = HubSpotClient(token).create("tickets", properties)
        except HubSpotError as e:
            yield self.create_text_message(f"HubSpot error: {e.message}")
            return

        yield self.create_text_message(f"Ticket created (id: {result.get('id')}).")
