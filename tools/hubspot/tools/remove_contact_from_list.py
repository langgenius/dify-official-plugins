...
class RemoveContactFromListTool(Tool):
    """Remove one or more contacts from a HubSpot contact list."""
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        try:
            result = HubSpotClient(token).remove_from_list(list_id, record_ids)
        except HubSpotError as e:
            yield self.create_text_message(f"HubSpot error: {e.message}")
            return

        yield self.create_text_message(f"Removed {len(record_ids)} contact(s) from list {list_id}.")
