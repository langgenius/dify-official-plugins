"""Unit tests for the opt-in Dify metadata helper used by the together.ai plugin.
...
These tests do not need a network or the together.ai SDK: the helper is pure
...
def test_helper_is_imported_in_llm_py() -> None:
    """The helper is wired into TogetherAILargeLanguageModel."""
    ...
    # Used at both call sites: in `_invoke` (after `_update_endpoint_url`) and
    # in `validate_credentials` (after `_update_endpoint_url`).
    assert source.count("apply_dify_headers_if_enabled(cred_with_endpoint)") == 2
    # Called only AFTER `_update_endpoint_url(credentials)` so the
    ...
    assert invoke_block.find(
        "self._update_endpoint_url(credentials=credentials)"
    ) < invoke_block.find("apply_dify_headers_if_enabled(cred_with_endpoint)")
