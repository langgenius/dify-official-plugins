"""Helpers for attaching Dify metadata to together.ai requests as request headers.

together.ai's plugin extends ``dify_plugin.OAICompatLargeLanguageModel``,
...
or other control characters. Values are truncated to 256 characters as
a hard cap (UUID app_ids are 36 chars, so the cap is a safety net
...
Caveats:
- together.ai's public API does not document these headers today, so
