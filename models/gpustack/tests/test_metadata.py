"""Unit tests for the opt-in Dify metadata helper used by the GPUStack plugin.
...
These tests do not need a network or the GPUStack SDK: the helper is pure
...
def test_helper_is_imported_in_llm_py() -> None:
    """The helper is wired into GPUStackLanguageModel.

    Note: GPUStack uses ``_get_compatible_credentials(credentials)`` which
    returns a *copy* of credentials with the ``endpoint_url`` normalized
    for the OAICompat base class. The helper must run on that copy
    (not on the caller's original ``credentials`` dict) so the OAICompat
    base class — which is invoked with ``compatible_credentials`` — picks
    up the Dify headers in its outbound request.
    """
    ...
    # Used at both call sites: in `_invoke` and in `validate_credentials`,
    # both called on the `compatible_credentials` copy (after
    # `_get_compatible_credentials(credentials)` so endpoint_url is settled).
    assert source.count("apply_dify_headers_if_enabled(compatible_credentials)") == 2
    # Called only AFTER `_get_compatible_credentials(credentials)` so the
    ...
    assert invoke_block.find(
        "self._get_compatible_credentials(credentials)"
    ) < invoke_block.find("apply_dify_headers_if_enabled(compatible_credentials)")
