"""Helpers for attaching Dify metadata to GPUStack requests as request headers.

GPUStack's plugin extends ``dify_plugin.OAICompatLargeLanguageModel``,
...
Caveats:
- GPUStack's public API does not document these headers today, so the
  ...
  longcat, zhinao, wenxin, lemonade, siliconflow).
