"""Unit tests for the opt-in Dify metadata helper used by the Huawei Cloud MAAS HK plugin.
...
`"enabled"` and a Dify `app_id` resolves. The Huawei Cloud MAAS HK plugin then
reads `credentials.get("extra_headers", {})` and passes it through to the
OAICompat base class's OpenAI SDK client constructor as
`default_headers=...`, which the OpenAI SDK sends on every outbound request.
...
runs entirely in memory. The integration with
`HuaweiCloudMaasHKLargeLanguageModel._invoke` and
`HuaweiCloudMaasHKLargeLanguageModel.validate_credentials` is verified by the
...
def test_helper_is_imported_and_used_in_llm() -> None:
    """The helper is wired into ``HuaweiCloudMaasHKLargeLanguageModel._invoke`` and ``validate_credentials``.

    Note: Huawei Cloud MAAS HK is a thin wrapper over
    ``dify_plugin.OAICompatLargeLanguageModel``. The OAICompat base
    class constructs an ``openai.OpenAI(**kwargs)`` client and
    forwards ``credentials['extra_headers']`` as ``default_headers=``
    on the OpenAI client constructor. The OpenAI SDK sends every
    entry of ``default_headers`` on each outbound request, which is
    the carrier for the Dify observability headers.

    Both ``_invoke`` and ``validate_credentials`` override the base
    class methods, call ``_add_custom_parameters`` and (for ``_invoke``)
    ``_add_function_call`` first, then call the helper, then delegate
    to the OAICompat base class via ``super()``. The helper runs
    AFTER ``_add_custom_parameters`` and ``_add_function_call`` so the
    dict is fully populated (including any function-calling
    credentials) before the OAICompat base class builds the OpenAI
    client.
    """
    ...
    assert llm_source.count("apply_dify_headers_if_enabled(credentials)") == 2
    # Both methods should delegate to the OAICompat base class.
    assert "super()._invoke(" in llm_source
    assert "super().validate_credentials(" in llm_source
