...
class TongyiLargeLanguageModel(LargeLanguageModel):
    ...
    def _generate(
        ...
        params = {"model": model, **model_parameters, **credentials_kwargs, **extra_model_kwargs}
    ...
    def _handle_generate_stream_response(
        ...
        for index, response in enumerate(responses):
            ...
            if resp_finish_reason is not None and resp_finish_reason != "null":
                ...
                if isinstance(resp_content, list):
                    resp_content = resp_content[0]["text"]
                assistant_prompt_message = AssistantPromptMessage(content=resp_content.replace(full_text, "", 1))
                full_text = resp_content
                yield LLMResultChunk(
                    model=model,
                    prompt_messages=prompt_messages,
    ...
    def _convert_tools(self, tools: list[PromptMessageTool]) -> list[dict]:
        ...
        for tool in tools:
            ...
            for p_key, p_val in properties.items():
                ...
                properties_definitions[p_key] = {"description": desc, "type": p_val["type"]}
            tool_definition = {
                "type": "function",
                "function": {
                    "name": tool.name,
