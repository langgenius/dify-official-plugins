...
class TongyiLargeLanguageModel(LargeLanguageModel):
    ...
    def _handle_error_response(self, status_code: int, message: str, model: str = None, request_id: str = None) -> None:
        ...
        """
        # Construct the error message including the request_id (if available).
        error_msg_parts = []
        if model:
            error_msg_parts.append(f"Failed to invoke model {model}")
        error_msg_parts.append(f"status code: {status_code}")
        
        # Add request_id to the error message.
        if request_id:
            error_msg_parts.append(f"request_id: {request_id}")
        
        error_msg_parts.append(f"message: {message}")
