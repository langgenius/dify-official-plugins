...
@pytest.mark.parametrize("model_name", QWEN37_FLASH_MODELS)
def test_qwen37_flash_schema(model_name: str) -> None:
    ...
    assert {ModelFeature.VISION, ModelFeature.VIDEO}.issubset(schema.features or [])
...
def test_qwen37_flash_models_are_ordered_together() -> None:
    position = _load_yaml(MODELS_DIR / "_position.yaml")
    indexes = [position.index(model_name) for model_name in QWEN37_FLASH_MODELS]
    assert indexes[1] == indexes[0] + 1


@pytest.mark.parametrize("model_name", QWEN37_FLASH_MODELS)
def test_qwen37_flash_defaults_to_non_thinking(model_name: str) -> None:
...
@pytest.mark.parametrize("model_name", QWEN37_FLASH_MODELS)
def test_qwen37_flash_forces_streaming_when_thinking(model_name: str) -> None:
...
@pytest.mark.parametrize("model_name", QWEN37_FLASH_MODELS)
def test_qwen37_flash_accepts_json_object_output(model_name: str) -> None:
