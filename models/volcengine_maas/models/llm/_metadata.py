"""Helpers for attaching Dify metadata to Volcengine MAAS requests as request headers.

Volcengine MAAS has two code paths (legacy v2 ``MaaSClient`` and modern v3
``ArkClientV3``). Both expose an ``extra_headers=`` slot on their chat
calls. The v3 client forwards the dict to the underlying OpenAI SDK's
``extra_headers=`` on each outbound request; the v2 vendored SDK
extends ``_call`` to merge the dict into the HTTP request headers.

The Dify app_id is propagated by writing the dify headers into
``credentials['extra_headers']`` and passing that dict as
``extra_headers=`` to the appropriate client call.
...
Caveats:
- Volcengine MAAS does not document these headers today, so the value
  is purely for observability / proxy-side propagation, identical to
  the bury-point headers used in the other LLM plugins (anthropic,
  gemini, stepfun, openai_api_compatible, tongyi, volcengine,
  zhipuai, minimax, cohere, mistralai, deepseek, fireworks, ollama,
  togetherai, moonshot, groq, xai, openrouter, sagemaker, yi, mimo,
  longcat, zhinao, wenxin, lemonade, siliconflow, gpustack, baichuan,
  upstage, llama_api, replicate, hunyuan, oci).
