"""Helpers for attaching Dify metadata to OCI requests as request headers.

OCI's plugin uses the OCI Python SDK (``oci.generative_ai_inference``)
with ``client.base_client.call_api(...)`` calls. The base client
takes a ``header_params`` dict that is forwarded on the outbound
request. The Dify app_id is propagated by writing the dify headers
into ``credentials['extra_headers']`` and merging them into the
``header_params`` dict right before the ``call_api`` invocation.
...
Caveats:
- OCI does not document these headers today, so the value is purely
  for observability / proxy-side propagation, identical to the
  bury-point headers used in the other LLM plugins (anthropic,
