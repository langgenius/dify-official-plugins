"""Local smoke test for the IO Intelligence plugin LLM implementation."""
import os
import sys
import traceback
...
from models.llm.llm import IonetLargeLanguageModel  # noqa: E402
...
def make_model() -> IonetLargeLanguageModel:
    return IonetLargeLanguageModel(model_schemas=[])
...
def test_sync(model_id: str) -> None:
    ...
    print(f"SYNC  {model_id:40s} ok content={text!r} usage={result.usage}")
...
def test_stream(model_id: str) -> None:
    ...
    print(f"STREAM {model_id:40s} ok chunks={chunks} text={preview!r}")


def test_validate(model_id: str) -> None:
    llm = make_model()
    llm.validate_credentials(model=model_id, credentials={"api_key": API_KEY})
    print(f"VALID {model_id:40s} ok")


def main() -> int:
    if not API_KEY:
        print("Set IONET_API_KEY to run this test.")
        return 1
    failed = 0
    print("=== validate_credentials ===")
    try:
        test_validate(MODELS_TO_TEST[0])
    except Exception:
        failed += 1
        traceback.print_exc()
    ...
    for mid in MODELS_TO_TEST:
        try:
            ...
            failed += 1
            print(f"SYNC  {mid:40s} FAIL {type(e).__name__}: {e}")

    print("\n=== stream invoke ===")
    for mid in MODELS_TO_TEST[:2]:
        try:
            test_stream(mid)
        except Exception as e:
            failed += 1
            print(f"STREAM {mid:40s} FAIL {type(e).__name__}: {e}")
