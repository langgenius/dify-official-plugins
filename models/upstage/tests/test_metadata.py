"""Unit tests for the opt-in Dify metadata helper used by the Upstage plugin.
...
`"enabled"` and a Dify `app_id` resolves. The Upstage plugin then reads
`credentials.get("extra_headers", {})` and passes it through to the OpenAI
SDK client constructor as `default_headers=...`, which the OpenAI SDK
forwards on every outbound request.
...
runs entirely in memory. The integration with `_to_credential_kwargs` (in
`models/_common.py`) is verified by the source-level guard tests at the
bottom of this file.
"""
...
def test_helper_is_imported_and_used_in_common() -> None:
    """The helper is wired into ``_CommonUpstage._to_credential_kwargs``.

    Note: Upstage uses the official ``openai`` Python SDK with
    ``client = OpenAI(**credentials_kwargs)``. The SDK supports custom
    default headers via the ``default_headers`` constructor kwarg, so
    this PR extends ``_to_credential_kwargs`` (in ``models/_common.py``)
    to call the helper and pick up
    ``credentials.get("extra_headers", {})`` as ``default_headers``.

    Both ``validate_credentials`` and ``_chat_generate`` in
    ``models/llm/llm.py`` call ``_to_credential_kwargs`` before
    constructing the OpenAI client, so the helper runs at both LLM
    call sites without any further wiring.
    """
    ...
    # Helper runs before the credentials dict is read back to populate
    # default_headers on the OpenAI client.
    assert "apply_dify_metadata_if_enabled(credentials)" in common_source
    assert "default_headers" in common_source
    assert common_source.count("default_headers") >= 2  # one in dict, one optional
    # The helper runs BEFORE the dict construction (so the helper's
    # mutation of credentials['extra_headers'] is observable when we
    # read it back into default_headers).
    assert common_source.index(
        "apply_dify_metadata_if_enabled(credentials)"
    ) < common_source.index('"default_headers": credentials.get("extra_headers", {})')
