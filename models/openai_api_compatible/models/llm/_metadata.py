"""Helpers for attaching Dify metadata to OpenAI-compatible requests as headers.

The openai_api_compatible plugin extends the SDK's
...
underlying OpenAI SDK is constructed with ``default_headers`` from
``credentials['extra_headers']`` only — so this plugin routes the Dify
app_id through the ``extra_headers`` credential instead. The SDK's
