"""Unit tests for the opt-in Dify metadata helper used by the Llama API plugin.
...
`"enabled"` and a Dify `app_id` resolves. The Llama API plugin then reads
...
`models/llm/llm.py`) is verified by the source-level guard tests at the
bottom of this file.
"""
...
def test_helper_is_imported_and_used_in_llm() -> None:
    """The helper is wired into ``LlamaApiLargeLanguageModel._to_credential_kwargs``.

    Note: Llama API uses the official ``openai`` Python SDK with
    ``client = OpenAI(**credentials_kwargs)``. The SDK supports custom
    default headers via the ``default_headers`` constructor kwarg, so
    this PR extends ``_to_credential_kwargs`` (in ``models/llm/llm.py``)
    to call the helper and pick up
    ``credentials.get("extra_headers", {})`` as ``default_headers``.

    Both ``validate_credentials`` and ``_chat_generate`` call
    ``_to_credential_kwargs`` before constructing the OpenAI client, so
    the helper runs at both LLM call sites without any further wiring.
    """
    ...
    assert "default_headers" in llm_source
    assert llm_source.count("default_headers") >= 2  # one in dict, one optional
    ...
    assert llm_source.index(
        "apply_dify_metadata_if_enabled(credentials)"
    ) < llm_source.index('"default_headers": credentials.get("extra_headers", {})')
