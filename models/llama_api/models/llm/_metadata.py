"""Helpers for attaching Dify metadata to Llama API requests as request headers.

Llama API's plugin uses the official ``openai`` Python SDK
(``client = OpenAI(**credentials_kwargs)``), which supports custom
default HTTP headers via the ``default_headers`` constructor kwarg.
The Dify app_id is propagated by writing the dify headers into
``credentials['extra_headers']`` and passing them to the OpenAI
client as ``default_headers=...``.
...
Caveats:
- Llama API's public docs do not document these headers today, so the
