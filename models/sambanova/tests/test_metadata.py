"""Unit tests for the opt-in Dify metadata helper used by the SambaCloud plugin.
...
`"enabled"` and a Dify `app_id` resolves. The SambaCloud plugin then reads
`credentials.get("extra_headers", {})` and passes it through to the OAICompat
base class's OpenAI SDK client constructor as `default_headers=...`, which the
OpenAI SDK sends on every outbound request.
...
`SambaNovaLargeLanguageModel._invoke` and
`SambaNovaLargeLanguageModel.validate_credentials` is verified by the
...
def test_helper_is_imported_and_used_in_llm() -> None:
    """The helper is wired into ``SambaNovaLargeLanguageModel._invoke`` and ``validate_credentials``.

    Note: SambaNova is a thin wrapper over
    ...
    class methods, call ``_add_custom_parameters`` first (to set
    ``mode`` and ``endpoint_url``), then call the helper, then
    delegate to the OAICompat base class via ``super()``.
    """
