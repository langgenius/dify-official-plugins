...
def test_apply_returns_new_dict_when_credential_missing():
    config: dict = {}
    result = apply_dify_metadata_if_enabled(config, {})
    assert result == {}
    # No-mutation contract: original input is left intact.
    assert config == {}


def test_apply_returns_new_dict_when_credential_disabled():
...
def test_apply_noop_without_session_context():
    ...
    # No app_id resolves, so no metadata is attached. The function
    # returns a new dict with the input's contents.
...
def test_apply_replaces_non_dict_metadata(monkeypatch):
    # If existing metadata is a string or list, the Dify dict takes
    # over rather than blowing up.
    import dify_plugin
...
def test_apply_preserves_other_extra_body_fields(monkeypatch):
    # Non-metadata fields in the extra_body are preserved.
    import dify_plugin
...
def test_llm_module_uses_dify_metadata_helper():
    # The wiring in llm.py must reach apply_dify_metadata_if_enabled so
    # the opt-in credential is honored on the request path. A
    # source-level guard catches accidental removal during refactors
    # of the call site.
    import inspect
    ...
    assert "apply_dify_metadata_if_enabled({}, credentials)" in source
    assert "extra_body=extra_body" in source
    assert "from ._metadata import apply_dify_metadata_if_enabled" in source


# --- Edge cases for the apply function ---


def test_apply_skips_when_no_app_id(monkeypatch):
    # When the session returns no app_id, the helper produces no
    # metadata and the call site sends extra_body without a metadata
    # key. The Ark API receives no metadata field.
    import dify_plugin
...
def test_apply_metadata_dify_keys_override_existing_collisions(monkeypatch):
    # If the caller already has a dify_app_id in metadata, the helper's
    # value wins (Dify is the source of truth for the app_id marker).
    import dify_plugin
