...
class TestContentToParts:
    def test_text_content(self):
        message = TextPromptMessageContent(data="Test text content")
        assert _content_to_part(message) == types.Part.from_text(
            text="Test text content"
        ), "TextPromptMessageContent should be converted to text Part."

    def test_multimodal_contents_with_url(self):
        cases = [
            TestCase(
                message=ImagePromptMessageContent(
                    format="jpeg", url="https://example.com/image.jpg", mime_type="image/jpeg"
                ),
            ...
            TestCase(
                message=AudioPromptMessageContent(
                    format="mp3", url="https://example.com/audio.mp3", mime_type="audio/mpeg"
                ),
                expected=types.Part.from_uri(
                    file_uri="https://example.com/audio.mp3", mime_type="audio/mpeg"
                ),
            ),
            TestCase(
                message=VideoPromptMessageContent(
                    format="mp4", url="https://example.com/video.mp4", mime_type="video/mp4"
                ),
                expected=types.Part.from_uri(
                    file_uri="https://example.com/video.mp4", mime_type="video/mp4"
                ),
            ),
            TestCase(
        ...
        for idx, c in enumerate(cases):
            assert (
                _content_to_part(c.message) == c.expected
    ...
    def test_multimodal_contents_with_base64(self):
        ...
        cases = [
            TestCase(
                ...
                expected=types.Part.from_bytes(data=binary_data, mime_type="image/jpeg"),
            ),
            TestCase(
                message=AudioPromptMessageContent(
                    format="mp3", base64_data=base64_data, mime_type="audio/mpeg"
                ),
                expected=types.Part.from_bytes(data=binary_data, mime_type="audio/mpeg"),
            ),
            TestCase(
                message=VideoPromptMessageContent(
                    format="mp4", base64_data=base64_data, mime_type="video/mp4"
                ),
                expected=types.Part.from_bytes(data=binary_data, mime_type="video/mp4"),
            ),
            TestCase(
                message=DocumentPromptMessageContent(
                    format="pdf", base64_data=base64_data, mime_type="application/pdf"
                ),
                expected=types.Part.from_bytes(data=binary_data, mime_type="application/pdf"),
            ),
        ]
    ...
    def test_unknown_content_type(self):
        cases = [
            PromptMessageContent(),
            MultiModalPromptMessageContent(format="fake", mime_type="text/plain", base64_data=""),
            "",
        ]
...
class TestConvertToContents:
    def test_convert_to_contents(self):
        ...
        expected: list[types.ContentDict] = [
            types.ContentDict(
                parts=[types.Part(text="User Prompt 1"), types.Part(text="User Prompt 2")],
                role="user",
            ),
            types.ContentDict(parts=[types.Part(text="Assistant Prompt 1")], role="model"),
            types.ContentDict(parts=[types.Part(text="User Prompt 3")], role="user"),
            types.ContentDict(parts=[types.Part(text="Tool Prompt 1")], role="model"),
            types.ContentDict(parts=[types.Part(text="User Prompt 4")], role="user"),
        ]
