"""Helpers for attaching Dify metadata to Replicate requests as request headers.

Replicate's plugin uses the official ``replicate`` Python SDK
(``client = ReplicateClient(api_token=..., timeout=30)``). The SDK accepts
``**kwargs`` which are forwarded to ``httpx.Client``. ``httpx.Client``
supports custom headers via the ``headers=`` constructor kwarg, so the
Dify app_id is propagated by writing the dify headers into
``credentials['extra_headers']`` and passing them to ``ReplicateClient``
as ``headers=...``.
...
Caveats:
- Replicate's public docs do not document these headers today, so the
