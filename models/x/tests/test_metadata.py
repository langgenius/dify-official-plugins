"""Unit tests for the opt-in Dify metadata helper used by the xAI Grok plugin.
...
`"enabled"` and a Dify `app_id` resolves. The xAI Grok plugin then reads
...
runs entirely in memory. The integration with `XAILargeLanguageModel._invoke`
and `XAILargeLanguageModel.validate_credentials` is verified by the
...
def test_helper_is_imported_and_used_in_llm() -> None:
    """The helper is wired into ``XAILargeLanguageModel._invoke`` and ``validate_credentials``.

    Note: xAI Grok is a thin wrapper over
    ...
    ``endpoint_url``, ``mode``, ``function_calling_type``, etc.),
    then call the helper, then delegate to the OAICompat base class
    via ``super()``.
    """
