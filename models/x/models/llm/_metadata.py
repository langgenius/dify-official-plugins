"""Helpers for attaching Dify metadata to xAI Grok requests as request headers.

xAI Grok's plugin is a thin wrapper over
``dify_plugin.OAICompatLargeLanguageModel`` (the OpenAI-compatible
base class). The base class constructs an ``openai.OpenAI(**kwargs)``
client which supports custom default HTTP headers via the
``default_headers`` constructor kwarg. The Dify app_id is propagated
by writing the dify headers into ``credentials['extra_headers']``
and passing them as ``default_headers=...`` on the OpenAI client.
...
Caveats:
- xAI Grok does not document these headers today, so the value is
  purely for observability / proxy-side propagation, identical to
  ...
  upstage, llama_api, replicate, hunyuan, oci, volcengine_maas,
  nvidia_nim, sambanova).
