"""Helpers for attaching Dify metadata to Hunyuan requests as request headers.

Hunyuan's plugin uses the official ``tencentcloud-sdk-python`` SDK.
The SDK's ``ChatCompletionsRequest`` carries a ``headers`` attribute
which is forwarded by ``HunyuanClient.ChatCompletions`` via
``_call_and_deserialize(..., headers=request.headers)``. The Dify
app_id is propagated by writing the dify headers into
``credentials['extra_headers']`` and assigning that dict to
``request.headers`` right before the call.
...
Caveats:
- Tencent Hunyuan's public docs do not document these headers today,
  so the value is purely for observability / proxy-side propagation,
