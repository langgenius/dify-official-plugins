...
class BedrockLargeLanguageModel(LargeLanguageModel):
    ...
    def _generate_with_converse(
        ...
        (inference_config, additional_model_fields) = self._convert_converse_api_model_parameters(
    ...
    def validate_credentials(self, model: str, credentials: dict) -> None:
        ...
        if "anthropic" in model:
            required_params = {"max_tokens": 32}
        elif "ai21" in model:
            required_params = {"temperature": 0.7, "topP": 0.9, "maxTokens": 32}
        try:
    ...
    def _handle_generate_stream_response(
        ...
        for event in stream:
            ...
            assistant_prompt_message = AssistantPromptMessage(content=content_delta or "")
            index += 1
