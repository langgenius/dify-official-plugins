"""Helpers for attaching Dify metadata to Baichuan requests as request headers.

Baichuan's plugin uses a vendored local SDK (``models.llm.baichuan_turbo.BaichuanModel``)
that issues ``requests.post()`` calls with headers built from
``request_headers`` (a property that returns the auth + content-type
pair). The Dify app_id is propagated by writing the dify headers
into ``credentials['extra_headers']`` and threading them through
``BaichuanModel.generate(..., extra_headers=...)`` so the SDK can
merge them into the outbound request headers before posting.
...
Caveats:
- Baichuan's public API does not document these headers today, so the
  ...
  longcat, zhinao, wenxin, lemonade, siliconflow, gpustack).
...
def apply_dify_metadata_if_enabled(credentials: dict) -> None:
    ...
    The return value is a body-field-style carrier: the caller is
    expected to read ``credentials.get('extra_headers', {})`` and pass
    it through to the downstream SDK call. For Baichuan the caller
    threads it into ``BaichuanModel.generate(..., extra_headers=...)``.
    """
    try:
