"""Unit tests for the opt-in Dify metadata helper used by the Baichuan plugin.
...
`"enabled"` and a Dify `app_id` resolves. The Baichuan LLM class then reads
`credentials.get("extra_headers", {})` and passes it through to
`BaichuanModel.generate(..., extra_headers=...)`, which merges the dict into
the outbound request headers alongside the auth and content-type headers.

When the credential is disabled, or `app_id` is missing / empty, the helper
does nothing and the request shape is unchanged.

These tests do not need a network or the Baichuan SDK: the helper is pure
and runs entirely in memory. The integration with `_generate` and
...
def test_helper_is_imported_in_llm_py() -> None:
    """The helper is wired into BaichuanLanguageModel.

    Note: baichuan uses a vendored local SDK
    (``models.llm.baichuan_turbo.BaichuanModel``) that issues
    ``requests.post()`` calls with headers built from a
    ``request_headers`` @property. The standard OAICompat
    ``credentials['extra_headers']`` carrier does not propagate to
    the SDK by itself, so this PR introduces a body-field-style
    helper (``apply_dify_metadata_if_enabled``) plus a small
    ``_build_dify_extra_headers(credentials)`` adapter in
    ``models/llm/llm.py`` that runs the helper and returns a fresh
    dict for ``BaichuanModel.generate(..., extra_headers=...)``.

    The source-level guard therefore checks for the adapter's call
    pattern rather than the bare ``apply_dify_metadata_if_enabled``
    call: it asserts the adapter is called at both call sites
    (``_generate`` and ``validate_credentials``) and that
    ``extra_headers=`` is passed into both ``BaichuanModel.generate``
    invocations.
    """
    ...
    # The adapter is called at both LLM call sites: in `_generate`
    # and in `validate_credentials`, before the BaichuanModel.generate
    # invocation that needs the headers.
    assert source.count("_build_dify_extra_headers(credentials)") == 2
    # Both BaichuanModel.generate calls thread the extra headers
    # through the new `extra_headers=` parameter.
    assert source.count("extra_headers=_build_dify_extra_headers(credentials)") == 2
