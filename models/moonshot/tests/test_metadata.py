"""Unit tests for the opt-in Dify metadata helper used by the Moonshot plugin.
...
These tests do not need a network or the Moonshot SDK: the helper is pure
...
def test_helper_is_imported_in_llm_py() -> None:
    """The helper is wired into MoonshotLargeLanguageModel."""
    ...
    # `_add_function_call`) and in `validate_credentials` (at the top, before
    # the endpoint probing).
    assert source.count("apply_dify_headers_if_enabled(credentials)") == 2
    # Called in `_invoke` AFTER `_add_custom_parameters(credentials)` and
    # `_add_function_call(model, credentials)` so the credentials dict is
    # fully set up before the helper mutates it.
    invoke_block = source[
