...
class ShengsuanyunTextEmbeddingModel(TextEmbeddingModel):
    def _invoke(
        self,
        model: str,
        credentials: dict,
        ...
        # init model client
        client = openai.OpenAI(**credentials_kwargs)
        ...
        # Ensure context_size and max_chunks are at least 1 to avoid range() errors
        if context_size <= 0:
            context_size = 1024  # Default context size for most embedding models
        if max_chunks <= 0:
            max_chunks = 1  # Process at least one chunk at a time
