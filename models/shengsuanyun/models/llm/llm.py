...
class ShengsuanyunLargeLanguageModel(OAICompatLargeLanguageModel):
    ...
    def validate_credentials(self, model: str, credentials: dict) -> None:
        ...
        try:
            ...
            # The parent class may try to access 'response' variable that doesn't exist
            logger.error(f"Credentials validation failed: {str(ex)}")
            raise
    ...
    def _handle_generate_response(
        ...
        try:
            ...
            raise InvokeError(f"Failed to parse response: {str(e)}")
        
        if "choices" not in response_json or not response_json["choices"]:
            raise InvokeError("Invalid response: missing choices")
