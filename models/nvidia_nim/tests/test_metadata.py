"""Unit tests for the opt-in Dify metadata helper used by the NVIDIA NIM plugin.
...
`"enabled"` and a Dify `app_id` resolves. The NVIDIA NIM plugin then reads
...
runs entirely in memory. The integration with `NVIDIANIMProvider._invoke`
and `NVIDIANIMProvider.validate_credentials` is verified by the source-level
...
def test_helper_is_imported_and_used_in_llm() -> None:
    """The helper is wired into ``NVIDIANIMProvider._invoke`` and ``validate_credentials``.

    Note: NVIDIA NIM is a thin wrapper over
    ...
    class methods, call the helper, then delegate to the base class
    via ``super()``.
    """
